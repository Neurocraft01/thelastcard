...
def validate_image_size(image):
    """Validate that uploaded image is under 500KB."""
    max_size = 500 * 1024  # 500KB
    if image.size > max_size:
        raise ValidationError(
            f'Image file size must be under 500KB. Your file is {image.size / 1024:.0f}KB.'
        )
...
class UserProfile(models.Model):
    ...
    # Profile Media (max 500KB per image)
    profile_photo = models.ImageField(
        upload_to='profiles/photos/',
        blank=True,
        null=True,
        validators=[validate_image_size],
        help_text='Max 500KB. JPG, PNG supported.'
    )
